...
public class MoonPhaseParser {
	public static void main(String[] args) throws IOException {
		...
		String urlstring = "http://aa.usno.navy.mil/cgi-bin/aa_moonphases.pl?year=" + year + 
				"&?month=" + month + "&?day=" + day;
		...
		if (hc.getResponseCode() != 200) {
			System.err.println(
					"Error: HTTP server response code is " + 
		...
		if (!hc.getContentType().startsWith("text/html")) {
			System.err.println(
					"Error: data received from " + urlstring + " is not text/html, but " + 
					hc.getContentType());
			console.close();
			return;
		}
		...
		for (String line : tableLines) {
			...
			if (line.indexOf(searchString) == 4) {
				foundPhase = "New Moon";
			} else if (line.indexOf(searchString) == 20) {
				foundPhase = "First Quarter";
			} else if (line.indexOf(searchString) == 36) {
				foundPhase = "Full Moon";
			} else if (line.indexOf(searchString) == 52) {
